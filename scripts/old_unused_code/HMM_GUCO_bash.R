...
dd <- dep$dep_id[idxDEP]
    ...
    saveRDS(m, paste0(out_dir, '/', args[1],'_HMM.RDS'))
    write.csv(dat, paste0(out_dir, '/', args[1],'.csv'), row.names = F)
    ...
    ggsave(paste0(out_dir, '/', args[1],'_plots.png'), p, units = 'in', width = 10, height = 5)

...
dlw_idx<-grep('C', dep$dep_id)
dep<-dep[-dlw_idx,]

rownames(dep) <- 1:nrow(dep)
dlw_idx<-grep('G', dep$dep_id)
dep<-dep[-dlw_idx,]

## Get list of raw data files to process
#
setwd("D:/2019 GN/ARDs")
fn <- list.files('D:/2019 GN/ARDs', full.names = T, pattern = '.csv')

# output directory

out_dir <- 'D:/2019 GN/ARDs/processed_acc'

...
#library(raster) # for depth from bathymetric map, not in my code
...
n.cores <- parallel::detectCores() #add -1 if in own computer
...
#rownames(dep) <- 1:nrow(dep)#rename rows in case there is previous handling (there was in my dep file)
#dep <- dep[(row.names(dep) %in% c("42","45", "46", "48")),] #selects specific rows by name

dep <- dep[c(42,45, 46, 48),] #select the true order not the rownames

#dlw_idx<-grep(c('18GUCO', '21GUCO', '22GUCO', '2PEBO'), dep$dep_id) #keep all Axxy deployments (will keep Camera and Guco deployments)
#dep<-dep[dlw_idx,]
...
#all axxy csv files that are available in the working directory

foreach (
  i=1:nrow(dep),
  #.combine = 'rbind', 
  .packages = c("dplyr", "seabiRds", 
                "imputeTS", "ggplot2",
                "momentuHMM", "cowplot"
  )
) %dopar% { 

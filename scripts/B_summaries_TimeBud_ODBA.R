...
system.time({
  ...
  for (i in 1:length(fn2)) {
    ...
    for (j in unique(set1$dep_id)) {
      ...
      #could also just use the function from seabiRds!!!!!!!!!
...
all_birds<- readRDS("C:/Users/francis van oordt/Documents/McGill/00Res Prop v2/Chap 1 - DLW axxy/axxy_depth_peru/data/processed_acc_third_run/all_birds_HMMS.RDS")

##########################################################################
### 2) Group DBA calculations for DBA energetics 3 sec not mean freq  (old 2 sec calculations) 
##########################################################################

#setwd("C:/Users/francis van oordt/Documents/McGill/00Res Prop v2/Chap 1 - DLW axxy/axxy_depth_peru/data/processed_acc_third_run/dba only fourth run")


### RE DIRECT TO FINAL DBA calculation from DRA

#create a list of names of pre produced outputs from HMM and axxy data
fn2<-list.files(path = "C:/Users/francis van oordt/Documents/McGill/00Res Prop v2/Chap 1 - DLW axxy/axxy_depth_peru/data/processed_acc_third_run/dba only 5th run 3secs" , full.names = TRUE, pattern = ".csv")

...
dives<-SUMMADIVES
dives <- as_tibble(dives) # this df was grouped when produced, need to tibble it. others wont need that, and this line would not affect it (I think)
...
for (i in unique(df$unique_trip)) { #loop for every unique trip per bird
  ...
  dives_one_trip <- dives_one_depID[which(dives_one_depID$startDive >= start & dives_one_depID$endDive <= end ),]#subset dives with dive_ids for the times of the unique trip
  
  if (nrow(dives_one_trip) > 0) {
    ...
    #ALERT!!!!
    #name columns inthe following tibble  according to previous summary block, in case you get blanks
    summ_dives_one_trip <- tibble('mean_MaxDepth' = NA, 'n()' = NA, 'mean_time' = NA)
  }
  
  one_trip_summs<-cbind(one_trip, summ_dives_one_trip) #bind GPS summary and new Dive summary for 1 trip
